    ...
    <section ref={containerRef} id="contact" className="bg-d0 border-t border-border py-16 lg:py-0 lg:min-h-screen pt-[100px] pb-12 px-7 md:px-[72px] flex flex-col justify-center relative">
      ...
      <div className="w-full max-w-[1260px] mx-auto flex flex-col lg:flex-row gap-8 lg:gap-12 items-center justify-between relative z-10">
        ...
        <div className="w-full lg:w-[45%] flex flex-col justify-center">
          ...
          <form onSubmit={handleSubmit} className="contact-reveal-item w-full max-w-[480px] flex flex-col gap-3">
            ...
            <div className="flex gap-4">
              <div className="flex-1 flex flex-col gap-1">
                <label className="text-[0.58rem] font-mono tracking-[0.2em] uppercase text-t-lo/70">Name</label>
                <input
                  type="text"
                  name="name"
                  required
                  ...
                  className="bg-transparent border-b border-border/80 focus:border-gold text-t-hi font-sans text-[0.92rem] outline-none pb-1 px-0 transition-all duration-300 placeholder:text-t-lo/25 focus:placeholder:opacity-0 disabled:opacity-60"
                />
                {errors.name && <span className="text-[0.62rem] text-gold font-mono uppercase tracking-wider mt-0.5">{errors.name}</span>}
              </div>

              <div className="flex-1 flex flex-col gap-1">
                <label className="text-[0.58rem] font-mono tracking-[0.2em] uppercase text-t-lo/70">Email</label>
                <input
                  type="email"
                  name="email"
                  required
            ...
            <div className="flex flex-col gap-1 relative" ref={dropdownRef}>
              ...
              <button
                ...
                className="w-full bg-transparent border-b border-border/80 focus:border-gold text-left text-t-hi font-sans text-[0.92rem] pb-1.5 px-0 transition-all duration-300 flex items-center justify-between cursor-none disabled:opacity-60"
              >
              ...
              {dropdownOpen && (
                <div className="absolute left-0 right-0 top-[102%] z-50 bg-d2 border border-border rounded-xl shadow-[0_8px_30px_rgba(201,169,110,0.15)] py-1.5 overflow-hidden animate-[fadeIn_0.2s_ease-out]">
                  {[
                    ...
                    <button
                      ...
                      className="w-full text-left px-4 py-2 hover:bg-gold hover:text-d0 font-sans text-[0.85rem] text-t-mid transition-colors duration-200 cursor-none flex items-center gap-2"
                    >
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[0.58rem] font-mono tracking-[0.2em] uppercase text-t-lo/70">Company <span className="text-t-lo/40">(optional)</span></label>
              <input
                type="text"
                name="company"
                disabled={isSubmitting || isSubmitted}
            ...
            {/* Message Input */}
            <div className="flex flex-col gap-1">
              <label className="text-[0.58rem] font-mono tracking-[0.2em] uppercase text-t-lo/70">Project</label>
              <textarea
                name="message"
                required
                disabled={isSubmitting || isSubmitted}
                ...
                rows={2}
                className="bg-transparent border-b border-border/80 focus:border-gold text-t-hi font-sans text-[0.92rem] outline-none pb-1 px-0 transition-all duration-300 placeholder:text-t-lo/25 focus:placeholder:opacity-0 disabled:opacity-60 resize-none"
              />
              ...
              <div className="flex items-center justify-between">
                {isSubmitted ? (
                  <button
                    type="button"
                    onClick={handleReset}
                    className="text-[0.65rem] font-mono tracking-wider uppercase text-gold/70 hover:text-gold transition-colors duration-200 underline underline-offset-4 cursor-none"
                  >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-border/80 hover:border-gold/60 text-t-mid hover:text-gold transition-colors duration-200 font-mono text-[0.55rem] uppercase tracking-wider cursor-none bg-d3/30"
                >
          ...
          {/* Details footer */}
          <div className="contact-reveal-item flex flex-wrap items-center gap-x-4 gap-y-1.5 mt-5 text-[0.55rem] font-mono tracking-[0.12em] text-t-lo/50 uppercase">
            <span>{siteConfig.availability.responseTime}</span>
            <span className="text-border/40">•</span>
            <span className="flex items-center gap-1.5">
              <span className="w-1 h-1 rounded-full bg-green animate-[sdot-ping_2s_infinite]"></span>
        ...
        <div className="w-full lg:w-[50%] flex flex-col relative items-center justify-center lg:mt-0 mt-8">
          ...
          <div className="contact-reveal-card absolute top-2 right-2 lg:top-4 lg:right-4 z-10 bg-d2 border border-border/80 rounded-2xl p-4 shadow-[0_8px_30px_rgba(201,169,110,0.06)] max-w-[230px] w-full flex flex-col gap-2.5">
            ...
            <div className="flex flex-col gap-2">
              <div>
                ...
                <p className="text-[0.72rem] text-t-hi font-medium mt-0.5">{siteConfig.location}</p>
              </div>
              <div>
                <p className="text-[0.55rem] font-mono tracking-wider uppercase text-t-lo">Response Time</p>
                <p className="text-[0.72rem] text-t-hi font-medium mt-0.5">{siteConfig.availability.responseTime}</p>
              </div>
              <div>
                <p className="text-[0.55rem] font-mono tracking-wider uppercase text-t-lo">Freelance</p>
                <p className="text-[0.72rem] text-t-hi font-medium mt-0.5">{siteConfig.availability.freelance}</p>
              </div>
            </div>
          </div>
          ...
          {/* Custom Quote Card */}
          <div className="contact-reveal-quote w-full max-w-[420px] p-3.5 border border-border/40 rounded-xl bg-d2/30 text-center relative mt-2">
